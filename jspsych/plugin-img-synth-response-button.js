var jsPsychImgSynthResponseButtons = (function (jspsych) {
  "use strict";
   ...
   * Uses ASDFGHJKL; keys for pitch and mouse Y position for filter cutoff (quality).
   *
  ...
  class ImgSynthResponsePlugin {
    ...
    trial(display_element, trial) {
      ...
      let activeNotes = {}; // Map of key -> {oscillator, filter, gain, frequency}
      let currentMouseY = 0;
      ...
      const html = `
        ...
        <div class="img-synth-container">
          ...
          <div class="img-synth-canvas-section">
            ...
            <div class="img-synth-keyboard-instruction">Use ASDFGHJKL; keys for pitch, mouse Y position for quality</div>
          </div>
        </div>
      ...
      const drawKeyboard = () => {
        ...
        for (let i = 0; i < NUM_KEYS; i++) {
          ...
          const key = KEYS[i];
          const isActive = activeNotes.hasOwnProperty(key);
          ...
          // Draw quality bar if key is active
          if (isActive) {
            const barHeight = instrumentCanvas.height - SQUARE_HEIGHT - 2 * SQUARE_SPACING - currentMouseY;
            if (barHeight > 0) {
              instrumentCtx.fillStyle = 'rgba(249, 89, 89, 0.5)';
              instrumentCtx.fillRect(x, currentMouseY, squareWidth, barHeight);
            }
          }
        }
      };

      // Calculate filter cutoff from Y position
      const calculateFilterCutoff = (y) => {
      ...
      const startNote = (key) => {
        ...
        // Stop all other notes (mono synth behavior)
        for (const activeKey in activeNotes) {
          if (activeKey !== key) {
            stopNote(activeKey);
          }
        }
        ...
        const frequency = KEY_FREQUENCIES[keyIndex];
      ...
      const stopNote = (key) => {
        ...
        note.gain.gain.linearRampToValueAtTime(0, now + fadeTime);
        note.oscillator.stop(now + fadeTime);
      ...
      const updateFilterCutoff = () => {
        for (const key in activeNotes) {
          ...
          note.filter.frequency.linearRampToValueAtTime(currentFilterCutoff, now + glideTime);
        }
      };
      ...
      const handleMouseMove = (event) => {
        ...
        currentFilterCutoff = calculateFilterCutoff(currentMouseY);
        
        // Update filter for all active notes
        if (Object.keys(activeNotes).length > 0) {
          ...
          interactions.push({
            ...
            activeKeys: Object.keys(activeNotes),
            t: timestamp
          });
        }
